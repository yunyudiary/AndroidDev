package com.jiangyy.android.dev.widget
...
import com.jiangyy.android.dev.R
import com.jiangyy.android.dev.anko.string2Millis
...
class CustomDatePicker
    ...
    private val mCallback: (Long) -> Unit,
    beginTimestamp: Long,
    endTimestamp: Long
) : View.OnClickListener, PickerView.OnSelectListener {
    ...
    init {
        if (beginTimestamp <= 0 || beginTimestamp >= endTimestamp) {
    ...
    override fun onClick(v: View) {
        when (v.id) {
            ...
            R.id.tv_confirm -> mCallback(mSelectedTime.timeInMillis)
        }
    ...
    fun setSelectedTime(dateStr: String, showAnim: Boolean): Boolean {
        return (
                canShow() && !TextUtils.isEmpty(dateStr) &&
                        setSelectedTime(
                            if(mCanShowPreciseTime) dateStr.string2Millis("yyyy-MM-dd HH:mm") else dateStr.string2Millis("yyyy-MM-dd"),
                            showAnim
                        )
                )
    }
